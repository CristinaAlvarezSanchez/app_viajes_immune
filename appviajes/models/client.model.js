...
const getByHotel = (hotelId) => {
    return db.query('SELECT c.nombre, c.apellidos, c.dni, v.fecha_salida as "fecha_entrada", v.fecha_vuelta as "fecha_salida" FROM tbi_clientes_viajes as tbi JOIN clientes AS c ON c.id = tbi.fk_cliente JOIN viajes AS v ON v.id = tbi.fk_viaje JOIN hoteles AS h ON h.id = v.hotel_id WHERE h.id = ?', [hotelId])
}
